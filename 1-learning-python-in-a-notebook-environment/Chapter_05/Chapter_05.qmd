...
- By combining these with statements that *control execution* we can implement complex behaviours

### Compound Statements

- We've seen basic single line statements in [Chapter 2](../Chapter_02/Chapter_02.qmd)
- Compound statements are sequences of statements grouped together with a *control structure* to execute together
...
- A compound statement contains
  ...
  2. Statements to be executed
     ...
     - Indentation level can be specified by the user but must be consistent
        - e.g. if we use four spaces for one level of indentation, all remaining must also be four spaces
     - Once the block is complete, we out-dent back to the previous level
...
- `if`, `while` and `for` loops all test their condition for a `True` value
...
- Python has three forms of equality operators
  ...
  2. `!=` the inequality operator
     - Works like the equality but checks that two items are *not* equal
...
- Comparison operators can be used to test the ordering of objects
    ...
    - For strings, it is dictionary ordered (on unicode value of characters)
...
- Act on `True` or `False` expressions to get a final `True` or `False` result
- Operators are,
...
- `and` and `or` have short-circuiting behaviour
    - Meaning the will stop evaluating sub-expressions once the final value has been determined
- If the first expression in `and` is `False` then it will return `False`
...
- When a variable is assigned in python the resulting expression returns `None`
- Python 3.8 introduces the walrus operator `:=`
    - When used as part of an expression it both performs the assignment and *returns* the assigned value
...
- This is a set of statements that is only executed if an expression tests `False`
- For example, consider the basic test if `snack` is contained in a set
    - We might want to print an alternative method if `snack` is not in the set
...
- If we want to combine multiple branches in the same block, we can use an `elif`
    - Shorthand for *else if*
    - Allow us to add sequential tests for if the first `if` (and preceding `elif`) tests fail
...
- `while` loops run a series of nested statements while their control expression evaluates to `True`
...
- For example, counting up to a certain number
    - Each iteration increases `counter` until the loop condition is `False` and the `while` loop ends
...
If the loop condition for a `while` loop is always `True` then the loop will never end. This is commonly referred to as an *infinite loop*
:::

#### `for` Loops

- The pattern of running a loop a fixed number of times is quite common
    - e.g. counting up to a certain value, iterating through a collection etc.
...
- For example, to iterate a fixed number of times we can use a `for` loop with a `range` object
...
- `break` and `continue` are specialised control statements that can be used within `while` and `for` loops
- `break` immediately exits from the current loop
- `continue` immediately goes to the next iteration of the loop
- In general, where possible prefer standard control flow since they obfuscate what's going on
    - But sometimes they are arguably the cleanest way to implement a solution

- Using a `break` statement we can stop otherwise apparently infinite loops
```{python}
...
- `continue` let's us move to the next loop iteration early, such as if there is an error condition
...
- Compound statements controlled by `if`, `while` or `for` statements allow for complex program logic
- They can used to optionally run blocks of code or repeat sections a certain number of times
...
3. Write a `for` loop that prints the numbers from $0$ to $9$, skipping $3$, $5$, $7$
