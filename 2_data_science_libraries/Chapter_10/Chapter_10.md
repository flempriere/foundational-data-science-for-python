...
```
    ...
    <div>
        ...
        <span id="d3df59cb-bb1a-4b50-b4a6-a79fc1f2fe29">Loading BokehJS ...</span>
    </div>

    Unable to display output for mime type(s): application/javascript, application/vnd.bokehjs_load.v0+json

    BokehUserWarning: ColumnDataSource's columns must be of the same length. Current lengths: ('x', 100), ('y1', 100), ('y2', 10)

    UnsetValueError: Circle(id='p1041', ...).radius doesn't have a value set
    ---------------------------------------------------------------------------
    UnsetValueError                           Traceback (most recent call last)
    Cell In[18], line 26
         22     tools=TOOLS, title="Brushing"
         23 )  # Create a second figure using the selected tools
         24 right.circle("x", "y1", source=source)  # Create a circle plot on the second figure
         25
    ---> 26 p = gridplot([[left, right]])  # Put the figures on a grid
         27 show(p)  # Show the grid

    File ~/work/foundational-data-science-for-python/foundational-data-science-for-python/.venv/lib/python3.14/site-packages/bokeh/layouts.py:314, in gridplot(children, sizing_mode, toolbar_location, ncols, width, height, toolbar_options, merge_tools)
        312 elif isinstance(item, LayoutDOM):
        313     if merge_tools:
    --> 314         for plot in item.select(dict(type=Plot)):
        315             toolbars.append(plot.toolbar)
        316             plot.toolbar_location = None

    File ~/work/foundational-data-science-for-python/foundational-data-science-for-python/.venv/lib/python3.14/site-packages/bokeh/models/plots.py:177, in Plot.select(self, *args, **kwargs)
        174 selector = _select_helper(args, kwargs)
        176 # Want to pass selector that is a dictionary
    --> 177 return _list_attr_splat(find(self.references(), selector))

    File ~/work/foundational-data-science-for-python/foundational-data-science-for-python/.venv/lib/python3.14/site-packages/bokeh/model/model.py:496, in Model.references(self)
    ...
    File ~/work/foundational-data-science-for-python/foundational-data-science-for-python/.venv/lib/python3.14/site-packages/bokeh/model/util.py:139, in collect_models(*input_values)
    ...
    File ~/work/foundational-data-science-for-python/foundational-data-science-for-python/.venv/lib/python3.14/site-packages/bokeh/model/util.py:119, in collect_filtered_models(discard, *input_values)
        117         ids.add(obj.id)
        118         collected.append(obj)
    --> 119         visit_immediate_value_references(obj, queue_one)
        121 return collected

    File ~/work/foundational-data-science-for-python/foundational-data-science-for-python/.venv/lib/python3.14/site-packages/bokeh/model/util.py:185, in visit_immediate_value_references(value, visitor)
    ...
    File ~/work/foundational-data-science-for-python/foundational-data-science-for-python/.venv/lib/python3.14/site-packages/bokeh/core/property/descriptors.py:282, in PropertyDescriptor.__get__(self, obj, owner)
